...
int	get_lineno(treenode* t) {
	return (t[LINE_NO].line_number);
}

void set_lineno(treenode* t, int l) {
	t[LINE_NO].line_number = l;
}

int	get_charno(treenode* t) {
	return (t[CHAR_NO].char_number);
}

void	set_charno(treenode* t, int cn) {
	t[CHAR_NO].char_number = cn;
}
...
int	get_binary_prio(treenode* t) {
	return (t[BINARY_PRIO].binary_priority);
}

void	set_binary_prio(treenode* t, int binary_priority) {
	t[BINARY_PRIO].binary_priority = binary_priority;
}
...
int	get_unary_prio(treenode* t) {
	return (t[UNARY_PRIO].unary_priority);
}

void	set_unary_prio(treenode* t, int priority) {
	t[UNARY_PRIO].unary_priority = priority;
}
...
int	get_num_of_actuals(treenode* t) {
	return (t[NUM_OF_ACTUALS].actuals);
}

void	set_num_of_actuals(treenode* t, int n) {
	t[NUM_OF_ACTUALS].actuals = n;
}
...
int	get_num_of_subscripts(treenode* t) {
	return (t[NUM_OF_SUBSCRIPTS].subscripts_count);
}

void	set_num_of_subscripts(treenode* t, int num) {
	t[NUM_OF_SUBSCRIPTS].subscripts_count = num;
}
...
int	get_blockno_of_proc(treenode* t) {
	return (t[BLOCKNO_OF_PROC].block_number_proc);
}

void	set_blockno_of_proc(treenode* t, int bn) {
	t[BLOCKNO_OF_PROC].block_number_proc = bn;
}

int	get_num_of_pars(treenode* t) {
	return (t[NUM_OF_PARS]).pars_count;
}

void	set_num_of_pars(treenode* t, int nop) {
	t[NUM_OF_PARS].pars_count = nop;
}

int	get_proc_level(treenode* t) {
	return (t[PROC_LEVEL].proc_level);
}

void	set_proc_level(treenode* t, int l) {
	t[PROC_LEVEL].proc_level = l;
}
...
int	get_block_number(treenode* t) {
	return (t[BLOCK_NUMBER].block_number);
}

void	set_block_number(treenode* t, int bn) {
	t[BLOCK_NUMBER].block_number = bn;
}
...
int	get_subtype_dims(treenode* t) {
	return (int)(t[SUBTYPE_DIMS].subtype_dims);
}

void	set_subtype_dims(treenode* t, int dims) {
	t[SUBTYPE_DIMS].subtype_dims = dims;
}
...
int	get_num_of_switch_elements(treenode* t) {
	return (t[NUM_OF_SWITCH_ELEMENTS].switch_count);
}

void	set_num_of_switch_elements(treenode* t, int nose) {
	t[NUM_OF_SWITCH_ELEMENTS].switch_count = nose;
}
...
treenode* get_binoperator(int binary_priority, char* rep) {
	treenode* h = bin_operators;

	while (h != (treenode*)0) {
		if ((get_opsym_for_bin(h) == rep) &&
			(get_binary_prio(h) == binary_priority))
			return h;
		h = get_nextnode(h);
	}
...
treenode* get_unoperator(int binary_priority, char* rep) {
	treenode* h = un_operators;

	while (h != (treenode*)0) {
		if ((get_opsym_for_un(h) == rep) &&
			(get_unary_prio(h) == binary_priority))
			return h;
		h = get_nextnode(h);
	}
	return (treenode*)0;
...
treenode* parse_expression(int binary_priority, struct symbol* s) {
	...
	if (symbol_value(s) == S_OPERATOR) {
		h1 = get_unoperator(binary_priority, store_in_tree(symbol_repr(s)));
		if (h1 != (treenode*)0) {
			h = new_node(UNARY_EXPRESSION, symbol_lineno(s), symbol_charno(s));
			next_symbol(s);
			set_un_operator(h, h1);
			set_unary_operand(h, parse_expression(binary_priority, s));
			return h;
		}
	}

	h = t = parse_expression(binary_priority + 1, s);
	while ((symbol_value(s) == S_OPERATOR) &&
		((h1 = get_binoperator(binary_priority, store_in_tree(symbol_repr(s)))))) {
		h = new_node(BINARY_EXPRESSION, symbol_lineno(s), symbol_charno(s));
		set_bin_operator(h, h1);
		set_binary_left_operand(h, t);
		next_symbol(s);
		set_binary_right_operand(h, parse_expression(binary_priority + 1, s));
		t = h;
	}
